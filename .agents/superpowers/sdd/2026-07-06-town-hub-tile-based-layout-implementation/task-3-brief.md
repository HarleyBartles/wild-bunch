...
Replace the old BuildingLayoutPattern-based catalog with the new PaletteSpec-based catalog. Add GetPaletteSpec method to return palette configuration for each BuildingLayoutPalette value.
...
## Interfaces
- Consumes: BuildingLayoutPalette enum (from Task 1), PaletteSpec record (from Task 2)
...
Replace the entire contents of BuildingLayoutCatalog.cs with:
...
Replace the entire contents of BuildingLayoutCatalogTests.cs with:
