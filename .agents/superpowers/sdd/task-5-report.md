...
- The ADR freshness table required by AGENTS.md (`docs/adr/INDEX.md` carries a per-ADR "Last checked" freshness table) does not exist in the current INDEX.md. This is a pre-existing gap, not introduced by this slice. Noting as a deferred self-healing item — the INDEX.md is generated by `scripts/generate_index_mesh.py` and may need updating to include the freshness table.
