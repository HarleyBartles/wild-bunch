...
- **Validation:** `dotnet build` and `dotnet test` (WildBunch.GameContent.Tests only — this refactor touches no integration/persistence surface) must pass.
...
Run the full validation suite to confirm zero behavior change.
...
Run a repo-wide search for the old names to confirm zero remaining references in tracked source/tests/docs (excluding `.agents/superpowers/sdd/historical/` and `.agents/superpowers/specs/` which are historical artifacts):

```bash
rg -n "SeedWorldBuilder|SeedWorldCatalog|IsCanonicalSeedWorld" --glob '!.agents/superpowers/sdd/historical/**' --glob '!.agents/superpowers/specs/**'
```

Expected: zero matches in `src/`, `tests/`, `docs/`, and `*.md` files outside the excluded historical paths.
