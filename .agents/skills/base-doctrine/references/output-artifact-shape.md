...
When a workspace reserves YAML for execution or continuity artifacts, do not use YAML blocks for ordinary assessments, buster summaries, planning notes, status updates, analysis, or route explanations.

In Wild Bunch and similar worker-control contexts, YAML-shaped blocks are reserved for:

- lawful send-ready dispatches;
- session busters or continuity artifacts;
...
A skill's local output template is not authority to override workspace artifact-shape law. If a skill says to use compact YAML but the active workspace treats YAML as dispatch/session-buster shaped, use a non-reserved shape and preserve the same semantics.
...
- A session buster may use the project's session-buster shape because continuity export is the requested artifact.
