#!/usr/bin/env pwsh
# Thin launcher for scaffold_contributing.py. Run with --help to see usage.
[CmdletBinding()]
...
if ($python -eq 'py') {
    & py -3 "$scriptDir\scaffold_contributing.py" @pyArgs
} else {
    & $python "$scriptDir\scaffold_contributing.py" @pyArgs
}
exit $LASTEXITCODE
