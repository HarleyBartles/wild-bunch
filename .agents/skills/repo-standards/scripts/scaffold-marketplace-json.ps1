#!/usr/bin/env pwsh
# Thin launcher for scaffold_marketplace_json.py. Run with --help to see usage.
[CmdletBinding()]
...
if ($python -eq 'py') {
    & py -3 "$scriptDir\scaffold_marketplace_json.py" @pyArgs
} else {
    & $python "$scriptDir\scaffold_marketplace_json.py" @pyArgs
}
exit $LASTEXITCODE
