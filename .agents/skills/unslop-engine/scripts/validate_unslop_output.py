...
def validate_prompts_only_manifest(manifest: dict, path: Path) -> list[str]:
    ...
    if (
        path.joinpath("samples").exists()
        or path.joinpath("analysis.md").exists()
        or path.joinpath("skill.md").exists()
        or path.joinpath("validation.md").exists()
        or path.joinpath("before-after").exists()
    ):
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--check",
        action="store_true",
        help="Validate CLI availability without an output directory (read-only).",
    )
    parser.add_argument("output", type=Path, nargs="?", help="Path to unslop-output directory.")
    args = parser.parse_args(argv)
    if args.check:
        print("OK: unslop output validator CLI check")
        return 0
