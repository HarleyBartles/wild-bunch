...
def fixture_samples(domain: str, dtype: str) -> list[Sample]:
    if dtype == "visual":
        body = [
            (
                '<html><body><nav class="navbar">Logo <button>Get Started</button></nav>'
                '<section class="hero"><h1>Unlock seamless growth</h1><p>In today\'s fast-moving '
                'landscape, teams need robust workflows.</p><div class="card">Trusted by 2,400+ teams</div>'
                '</section><section class="pricing"><div class="card">Most Popular</div></section></body></html>'
            ),
            (
                '<html><body><nav class="navbar">Brand <button>Book a Demo</button></nav>'
                '<section class="hero"><h1>Transform your workflow</h1><p>At its core, this platform helps '
                'teams navigate complexity.</p><div class="card">Trusted by leading teams</div></section>'
                '<section class="testimonial">Loved by operators</section></body></html>'
            ),
            (
                '<html><body><header class="navbar">Product <button>Start Free</button></header>'
                '<main class="hero"><h1>Seamless operations for modern teams</h1><p>The bottom line: '
                'robust tooling unlocks productivity.</p><div class="pricing card">Most Popular plan</div>'
                "</main></body></html>"
            ),
        ]
    else:
        body = [
            (
                f"In today's {domain} landscape, teams need robust processes. Let's dive into why this matters. "
                "The bottom line is that seamless workflows unlock better outcomes."
            ),
            (
                f"At its core, {domain} is about helping teams navigate complexity. Here's the thing: robust "
                "systems are essential. Let's unpack the practical steps."
            ),
            (
                f"In an increasingly complex {domain} world, it is worth noting that teams need seamless "
                "collaboration. The reality is that the right ecosystem can unlock progress."
            ),
            (
                f"The bottom line for {domain}: avoid vague landscapes and focus on concrete work. But here's "
                "why this matters: robust planning changes outcomes."
            ),
        ]
...
def load_samples(
    samples_dir: Path | None, inline_samples: list[str], dtype: str, use_fixtures: bool, domain: str
) -> list[Sample]:
    loaded: list[Sample] = []
    if samples_dir:
...
def ensure_safe_output_dir(out: Path) -> None:
    ...
    if resolved_out == cwd or cwd.is_relative_to(resolved_out):
        raise ValueError(
            f"Refusing to clean output directory {out!s}: choose a dedicated generated directory outside "
...
def write_analysis(out: Path, result: dict[str, object]) -> None:
    ...
    lines.extend(
        [
            ...
            (
                f"The `{domain}` samples show defaults that should be removed before this profile is reused. "
                f"Prefer constraints that make future outputs more concrete for `{domain}` instead of replacing "
                "these defaults with another generic tone."
            ),
            "",
            "## Validation notes",
    ...
    with (out / "analysis.md").open("w", encoding="utf-8", newline="\n") as f:
        f.write("\n".join(lines) + "\n")
...
def make_avoid_bullets(result: dict[str, object]) -> list[str]:
    ...
    for item, _count in list(result["html_structure"])[:8]:  # type: ignore[index]
        bullets.append(
            f"- Do not rely on the repeated visual/code structure {item} unless the prompt specifically requires it."
        )
    if len(bullets) < 8:
        domain = str(result["domain"])
        bullets.extend(
...
def write_skill(out: Path, result: dict[str, object]) -> None:
    ...
    lines = [
        ...
        *bullets,
        ...
        "## Review rule",
        (
            "If a future output starts to match several avoided patterns, stop and rewrite from the domain "
            "facts rather than polishing the generic draft."
        ),
    ]
    with (out / "skill.md").open("w", encoding="utf-8", newline="\n") as f:
        f.write("\n".join(lines) + "\n")
...
def visual_smoke(dtype: str) -> dict[str, object]:
    ...
    if importlib.util.find_spec("playwright") is None:
        return {
            "requested": True,
            "status": "skipped",
            "reason": "Playwright Python package missing",
            "browser_candidates": [],
        }
    browser_candidates = [
        candidate
...
def validate_result(
    out: Path, domain: str, dtype: str, sample_count: int, visual_status: dict[str, object]
) -> tuple[bool, list[str]]:
    issues: list[str] = []
...
def write_validation(out: Path, passed: bool, issues: list[str], visual_status: dict[str, object]) -> None:
    ...
    with (out / "validation.md").open("w", encoding="utf-8", newline="\n") as f:
        f.write("\n".join(lines) + "\n")
...
def write_before_after_note(out: Path, prompts: list[dict[str, object]], skill_path: Path) -> None:
    ...
    with (comparison / "test-prompt.txt").open("w", encoding="utf-8", newline="\n") as f:
        f.write(str(prompt) + "\n")
    with (comparison / "README.md").open("w", encoding="utf-8", newline="\n") as f:
        f.write(
            "\n".join(
                [
                    "# Before/after review",
                    "",
                    (
                        "This package does not call a model provider directly. To compare before and after output, "
                        "run the test prompt once without the generated profile and once with the profile prepended."
                    ),
                    "",
                    "- Test prompt: `test-prompt.txt`",
...
def run(args: argparse.Namespace) -> int:
    ...
    visual_status = (
        asyncio.run(render_visual_samples(out, smoke)) if args.type == "visual" else {"status": "not_requested"}
    )
