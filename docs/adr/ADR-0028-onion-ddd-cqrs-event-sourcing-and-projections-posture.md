...
- 2026-07-18 - live (BUNCH-77/Plan C): `RehydrateFromEvents` wired as the production load path in `EfGameSessionRepository`. `GetByIdAsync` now selects between the snapshot fast path (when `SnapshotVersion == StreamVersion`) and full replay via `LoadFromEventsAsync` (when the snapshot is stale or components are missing). `LoadFromEventsAsync` reconstructs the world from `WorldGenerated`, rehydrates the aggregate via `RehydrateFromEvents`, rebuilds diary days via `TravelDiaryDayProjector`, and sets committed events for projection-backed read paths. `StoreAsync` updated to store `AllEvents` for new sessions (not just `UncommittedEvents`) and to always set `SnapshotVersion` (even when no events are produced, e.g. prepped sessions). `FullReplayEqualityTests` prove snapshot load == event replay for purchase flow, journey cycle, and missing-snapshot fallback. The snapshot is now a true shortcut cache, not a requirement — the BUNCH-3 review trigger is satisfied.
- 2026-07-18 - live (BUNCH-77/Plans D-E): Event versioning and load funnel infrastructure completed. `PayloadUpcasterRegistry` (Plan D) provides event upcasting with fail-closed version checks — event versions are derived from upcaster count (no hand-edited version registry), future versions throw, and non-contiguous upcaster chains throw at construction. `PersistedPayloadLoader` (Plan E) is the single load funnel enforcing version checks on all persisted payloads — events are upcasted via the registry, components and diary days are version-checked against `ProjectionVersions` (hand-edited per-projection version constants), and stale projections are rebuilt from the event stream. `GameSessionJsonSerializer`'s deserialize methods are `internal` — no code outside `WildBunch.Persistence` can bypass the funnel. All three load paths (`EfGameSessionRepository.LoadFromEventsAsync`, `GetEventStreamAsync`, `GameSessionReadStoreLoader.LoadStoreAsync`) and `GameSessionComponentPayloads` route through the funnel. Projection write-side stamping uses `ProjectionVersions.ForComponent(name)` / `ProjectionVersions.DiaryDay` instead of hardcoded values. `ProjectionVersionCompletenessTests` and `VersionMismatchBehaviorTests` guard the versioning infrastructure.
...
- ~~When BUNCH-3 lands full replay-from-events and the snapshot becomes pure cache.~~ (Satisfied 2026-07-18: `LoadFromEventsAsync` is the production full-replay load path; `FullReplayEqualityTests` prove snapshot load == event replay.)
