...
public sealed class SeedWorldBuilderTests
{
    ...
    public void CreateCanonicalWorldProducesEightTownsFromNamePool()
    {
        ...
        Assert.Equal(7, world.Trails.Count);
    ...
    public void FrontierVariantProducesDifferentTerrainThanCanonical()
    {
        ...
        // Frontier variant should produce different town coordinates, which may result in
        // different trail topology and/or terrain/water assignments
        var canonicalCoords = string.Join("|", canonicalWorld.Towns
            .OrderBy(t => t.Id.Value)
            .Select(t => $"{t.MapX},{t.MapY}"));
        var frontierCoords = string.Join("|", frontierWorld.Towns
            .OrderBy(t => t.Id.Value)
            .Select(t => $"{t.MapX},{t.MapY}"));

        Assert.NotEqual(canonicalCoords, frontierCoords);
    }
    ...
    public void BuilderCreatesWorldFromSeedWorldTemplate()
    {
        ...
        // Trails are generated by SeedWorldBuilder after town coordinates settle
        // SeedWorld no longer pre-generates trails
        Assert.Equal(7, world.Trails.Count); // n-1 edges for 8 towns
    }
    ...
    private static SeedWorld CreateSeedWorldWithPalettes(ProsperityPalette prosperity, ServicesPalette services)
    {
        ...
        // Trails are now generated by SeedWorldBuilder using geometry-first approach
        // SeedWorld.Trails is obsolete and will be removed in a future update
    ...
    public void CreateWorld_NonBoringEntropy_TrimsOutlierTrails_NotTowns()
    {
        ...
        // Geometry-first trail generation produces connected graphs for all entropy modes
        // Trail counts may vary due to different town coordinates from entropy mutations
        // but all should produce valid connected graphs
        Assert.True(boringWorld.Trails.Count > 0, "Boring mode should produce trails");
        Assert.True(classicWorld.Trails.Count > 0, "Classic mode should produce trails");
        Assert.True(adventurousWorld.Trails.Count > 0, "Adventurous mode should produce trails");
    }
    ...
    public void CreateWorld_BoringMode_DoesNotTrimOutlierTowns()
    {
        ...
        // Trails should be generated using geometry-first approach
        // Just verify that trails exist and form a connected graph
        Assert.True(boringWorld.Trails.Count > 0);
        Assert.True(IsConnected(boringWorld.Trails, boringWorld.Towns.Count));
    }
