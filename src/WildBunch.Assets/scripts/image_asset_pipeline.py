...
def _parse_args() -> argparse.Namespace:
    ...
    stage_tiles = subparsers.add_parser(
        "stage-tiles",
        help="Scale a tile tree down to the staging canvas without trimming or recentering",
    )
    ...
    promote_tiles = subparsers.add_parser(
        "promote-tiles",
        help="Copy a staged tile tree into the matching production tree without resizing",
    )
    promote_tiles.add_argument("--input-root", required=True, type=Path, help="Tile staging root")
    promote_tiles.add_argument("--out-root", required=True, type=Path, help="Tile production root")
    ...
    promote = subparsers.add_parser(
        "promote-sprites",
        help="Normalize a pipeline tree into the matching production tree",
    )
    promote.add_argument("--input-root", required=True, type=Path, help="Pipeline root to promote from")
    promote.add_argument("--out-root", required=True, type=Path, help="Production root to promote into")
    ...
    tiles = subparsers.add_parser(
        "tiles",
        help="Run the tile pipeline sequentially, with optional stage-only or promote-only modes",
    )
    tiles.add_argument("--input-root", type=Path, help="Tile source root")
    tiles.add_argument("--staging-root", type=Path, help="Tile staging root")
    tiles.add_argument("--production-root", type=Path, help="Tile production root")
    tiles.add_argument("--canvas-width", type=int, default=80)
    tiles.add_argument("--canvas-height", type=int, default=50)
    tiles.add_argument(
        "--stage-only",
        action="store_true",
        help="Run only the source-to-staging step",
    )
    tiles.add_argument(
        "--promote-only",
        action="store_true",
        help="Run only the staging-to-production step",
    )
...
def stage_tiles(input_root: Path, output_root: Path, config: PipelineConfig) -> int:
    if not input_root.exists():
        raise SystemExit(f"Input root does not exist: {input_root}")
    ...
    for source_path in sorted(input_root.rglob("*.png")):
        ...
        with Image.open(source_path) as image:
            resample = getattr(Image, "Resampling", Image).LANCZOS
            cut = image.resize((config.canvas_width, config.canvas_height), resample)
...
def main() -> int:
    ...
    if args.command == "tiles":
        if args.stage_only and args.promote_only:
            raise SystemExit("--stage-only and --promote-only are mutually exclusive")
        ...
        if not args.promote_only:
            if args.input_root is None or args.staging_root is None:
                raise SystemExit("--input-root and --staging-root are required unless --promote-only is set")
            staged = stage_tiles(args.input_root, args.staging_root, config)
            print(f"Staged {staged} PNG files from {args.input_root} to {args.staging_root}")

        if not args.stage_only:
            if args.staging_root is None or args.production_root is None:
